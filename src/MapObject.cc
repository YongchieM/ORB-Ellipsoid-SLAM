...
void MapObject::UpdateRotation(MapObject *otherLocalObject)
{
    g2o::ellipsoid other_ellipsoid = otherLocalObject->GetWorldPos();
    g2o::ellipsoid this_ellipsoid = this->GetWorldPos();
    Eigen::Matrix<double,9,1> other_vec = other_ellipsoid.toMinimalVector();
    Eigen::Matrix<double,9,1> this_vec = this_ellipsoid.toMinimalVector();

    Eigen::Matrix<double,9,1> new_vec = other_vec*1.0/3.0 + this_vec*2.0/3.0;

    rotation_avg = Eigen::Vector3d(new_vec(3),new_vec(4),new_vec(5));
    rotation_averaged = true;
}
...
void MapObject::SetWorldPos(const g2o::ellipsoid &Pos)
{
...
g2o::ellipsoid MapObject::GetWorldPos()
{
	unique_lock<mutex> lock(mMutexPos);
	if (is_dynamic)
...
g2o::ellipsoid MapObject::GetWorldPosBA()
{
	unique_lock<mutex> lock(mMutexPos);
