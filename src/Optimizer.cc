...
void Optimizer::LocalBACameraPointObjects(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, bool fixCamera, bool fixPoint)
{
    ...
    for (vector<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++)
    {
        vector<MapObject *> vpMOs = (*lit)->ellipsoids_landmark;
...
#define ObjectFixScale // use when scale is provided and fixed. such as KITTI
...
#ifdef ObjectFixScale
    typedef g2o::VertexEllipsoidFixScale g2o_object_vertex;
    typedef g2o::EdgeSE3EllipsoidFixScaleProj g2o_camera_obj_2d_edge;
#else
    typedef g2o::VertexEllipsoid g2o_object_vertex;
    typedef g2o::EdgeSE3EllipsoidProj g2o_camera_obj_2d_edge;
#endif
    ...
    {
        MapObject *pMObject = *lit;
        g2o::ellipsoid ellipsoid_pose = pMObject->GetWorldPos();
        ...
        if (scene_unique_id == kitti)
            ...
            vObject->fixedscale = Eigen::Vector3d(car_half_length,car_half_width,car_half_height);
        else
            std::cout << "Please see cuboid scale!!!, otherwise use VertexCuboid()" << std::endl;
        ...
        if (scene_unique_id == kitti)
        {
            if (!build_worldframe_on_ground)
            {
                ...
                ellipsoid_pose.setTranslation(Vector3d(ellipsoid_pose.translation()(0), cam_height + 1.0, ellipsoid_pose.translation()(2)));
            }
            else
            {
                float cam_height = pKF->GetCameraCenter().at<float>(2);
                ellipsoid_pose.setTranslation(Vector3d(ellipsoid_pose.translation()(0), ellipsoid_pose.translation()(1), cam_height - 1.0));
            }
            if (vObject->fixedscale(0) > 0) // even later didn't actually optimize, change the scale. just for visualization.
                ellipsoid_pose.setScale(vObject->fixedscale);
        }
#endif
        vObject->setEstimate(ellipsoid_pose);
    ...
    if (1)
    {
        ...
        for (size_t i = 0; i < lLocalMapObjects.size(); i++)
        {
            ...
            if ((all_object_ba_points[i].size() > 5) && 1) // whether want to initialize object position to be center of points
            {
                ...
                g2o::ellipsoid tempellipsoid = vObject->estimate();
                tempellipsoid.setTranslation(mean_point_final);
                vObject->setEstimate(tempellipsoid);
            }
        }
        ...
        for (size_t i = 0; i < lLocalMapObjects.size(); i++) // no need to optimize all objects...., use local KF's map objects?
        {
            ...
            {
#ifdef ObjectFixScale
                g2o::EdgePointEllipsoidOnlyObjectFixScale *e = new g2o::EdgePointEllipsoidOnlyObjectFixScale();
#else
                g2o::EdgePointEllipsoidOnlyObject *e = new g2o::EdgePointEllipsoidOnlyObject();
#endif
                for (size_t j = 0; j < all_object_ba_points[i].size(); j++)
                ...
                if (e->object_points.size() > 10)
                {
                    ...
                    if (scene_unique_id == kitti)
                    {
                        ...
                        e->prior_object_half_size = Eigen::Vector3d(car_half_length,car_half_width,car_half_height);
                    }
                    optimizer.addEdge(e);
                }
            }
            if (0) // each object is connect to one point. optimize both point and object
            {
                if (pMO->used_points_in_BA_filtered.size() > 10)
                    ...
                    {
                        g2o::EdgePointEllipsoid *e = new g2o::EdgePointEllipsoid();
        ...
        if (whether_want_camera_obj)
            for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
            {
                ...
                for (unordered_map<KeyFrame *, size_t>::const_iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
                {
                    ...
                    if (!pKFi->isBad())
                    {
                        const MapObject *local_object = pKFi->local_ellipsoids[mit->second];
    ...
    */
    ...
    for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(), lend = lLocalMapObjects.end(); lit != lend; lit++)
    {
        ...
        {
            if(pMObject->rotation_averaged)
            {
                g2o::ellipsoid obj = vObject->estimate();
                Eigen::Matrix<double,9,1> obj_vec  = obj.toMinimalVector();
                double obj_yaw = obj_vec(5);

                Eigen::Vector3d avg_rpy = pMObject->rotation_avg;
                double avg_yaw = avg_rpy(2);
                double yaw_diff = std::fabs(obj_yaw-avg_yaw);
                //std::cout<<"yaw diff: "<<yaw_diff<<std::endl;
                if(yaw_diff > orientation_thresh)
                {
                    obj.setRotation(avg_rpy);
                    pMObject->SetWorldPos(obj);
                }
            }
        }
    }
