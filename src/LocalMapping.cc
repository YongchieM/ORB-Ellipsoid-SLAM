    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpMap->KeyFramesInMap()>5) // BA total object
                {
                ...
                else    // normal BA, only points
                {
                    std::cout << "Optimizer: BA with only points" << std::endl;
                    Optimizer::LocalBundleAdjustment(mpCurrentKeyFrame,&mbAbortBA, mpMap);
                }
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    std::cout << "Mapping: recent buffer pts:  " << mlpRecentAddedMapPoints.size() << std::endl;
