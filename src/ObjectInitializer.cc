...
Matrix<double,9,1> initializeWithYaw(cv::Mat &all_lines_mat, Vector3d &obj_size, double height, SE3Quat campose_cw, Vector4d &bbox_value, Matrix3d &calib)
{
    Eigen::VectorXd pose = campose_cw.inverse().toVector();
    ...
    Vector2d lefttop_expan = Vector2d(left_x_raw-3, top_y_raw-3);
    Vector2d rightbottom_expan = Vector2d(right_x_raw+3, down_y_raw+3);
    Vector2d widthheight_expan = rightbottom_expan-lefttop_expan;
    ...
    cv::Rect object_bbox = cv::Rect(lefttop_expan(0),lefttop_expan(1),widthheight_expan(0),widthheight_expan(1));
    //dist map
    cv::Mat im_canny;
    cv::Canny(gray_img(object_bbox),im_canny,80,200);
    cv::Mat dist_map;
    cv::distanceTransform(255-im_canny,dist_map,CV_DIST_L2,3);
    ...
    //find best rotation
    double rot_final=0;
    std::vector<Eigen::Vector3d> yaw_error;
    std::vector<std::pair<double,double>> yaw_error_iou;
    for(double yaw=-3.14159; yaw<=3.14159; yaw+=0.1)
    {
        double dis=0;
        for(int i=0; i<4; i++)
        ...
        MatrixXd corners_2d = e_temp.projectOntoImage(campose_cw,calib);
        MatrixXd corners_2d_shift(2,8);
        corners_2d_shift.row(0) = corners_2d.row(0).array()-lefttop_expan(0);
        corners_2d_shift.row(1) = corners_2d.row(1).array()-lefttop_expan(1);
        ...
        MatrixXi visible_edge_pt_ids1(9,2);
        MatrixXi visible_edge_pt_ids2(7,2);
        visible_edge_pt_ids1 << 5,6, 6,7, 7,8, 8,5, 1,5, 2,6, 3,7, 2,3, 2,1;
        visible_edge_pt_ids2 << 5,6, 6,7, 7,8, 8,5, 2,6, 1,5, 1,2;
        visible_edge_pt_ids1.array() -= 1;
        visible_edge_pt_ids2.array() -= 1;
        double sum_dist1 = box_edge_sum_dists(dist_map,corners_2d_shift,visible_edge_pt_ids1);
        double sum_dist2 = box_edge_sum_dists(dist_map,corners_2d_shift,visible_edge_pt_ids2,true);
        double sum_dist = std::min(sum_dist1,sum_dist2);
        //std::cout<<"dist_error: "<<sum_dist<<std::endl;

        if(!few_edges)
            yaw_error.push_back(Eigen::Vector3d(yaw,total_angle_diff,sum_dist));
        yaw_error_iou.push_back(std::make_pair(yaw,bb_iou));
    }
    ...
    if(!yaw_error.empty())
    {
        ...
        double min_ang_error = 1e6;
        double max_ang_error = -1;
        for(int i=0; i<yaw_error.size(); i++)
        {
            double temp_dist = yaw_error[i][2];
            double temp_ang = yaw_error[i][1];
            min_dist_error = std::min(min_dist_error,temp_dist);
            max_dist_error = std::max(max_dist_error,temp_dist);
            min_ang_error = std::min(min_ang_error,temp_ang);
            max_ang_error = std::max(max_ang_error,temp_ang);
        }

        double weight_vp_angle = 0.7;
        vector<pair<double,double>> combined_scores(yaw_error.size());
        double dist_score,ang_score,combined_score;
        for(int i=0; i<yaw_error.size(); i++)
        {
            dist_score = (yaw_error[i][2]-min_dist_error)/(max_dist_error-min_dist_error);
            if((max_ang_error-min_ang_error)>0)
            {
                ang_score = (yaw_error[i][1]-min_ang_error)/(max_ang_error-min_ang_error);
                combined_score = (dist_score+weight_vp_angle*ang_score)/(1+weight_vp_angle);
            }
            else
                combined_score = (dist_score+weight_vp_angle*ang_score)/(1+weight_vp_angle);

            //std::cout<<"combined score: "<<combined_score<<std::endl;
            combined_scores[i] = make_pair(yaw_error[i][0],combined_score);
        }

        for(int i=0; i<combined_scores.size(); i++)
        {
            if(combined_scores[i].second < min_score)
            {
                min_score = combined_scores[i].second;
                rot_final = combined_scores[i].first;
            }
        }
    }
    if(yaw_error.empty() || min_score>0.5)
    {
        double max_iou = -1;
        for(int i=0; i<yaw_error_iou.size(); i++)
        {
            if(yaw_error_iou[i].second > max_iou)
            {
                max_iou = yaw_error_iou[i].second;
                rot_final = yaw_error_iou[i].first;
            }
        }
    }
