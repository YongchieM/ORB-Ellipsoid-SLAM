    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    whether_read_offline_object = true;
    whether_save_online_detected_objects = false;
    whether_save_final_optimized_objects = false;
	if (whether_detect_object)
	{
		// n.param<bool>("use_truth_trackid", use_truth_trackid, false);
		if (!whether_read_offline_object)
		{
		...
		if (!whether_read_offline_object)
		{
			...
			if (whether_save_online_detected_objects)
			{
				std::string save_object_pose_txt = base_data_folder + "/slam_output/orb_live_pred_objs_temp.txt";
				save_online_detected_objects.open(save_object_pose_txt.c_str());
			}
		}
		if (whether_read_offline_object)
			ReadAllObjecttxt();
	...
	if (whether_detect_object)
	{
		...
		if (whether_save_final_optimized_objects)
		{
...
void Tracking::CreateInitialMapMonocular()
{
	...
	if (whether_detect_object)
	{
        ...
        DetectObject(pKFini);
		AssociateObjects(pKFini);
		DetectObject(pKFcur);
		AssociateObjects(pKFcur);
	}
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    //condition for new object
    Eigen::MatrixXd pred_frame_objects;
    pred_frame_objects = all_offline_local_objects[mCurrentFrame.mnId];
    std::vector<cv::Rect> rects;
    for(int i=0; i<pred_frame_objects.rows(); i++)
    {
        cv::Rect rect(pred_frame_objects(i,7),pred_frame_objects(i,8),pred_frame_objects(i,9),pred_frame_objects(i,10));
        rects.push_back((rect));
    }
    std::vector<cv::Rect> last_rects;
    g2o::SE3Quat campose_cw = Converter::toSE3Quat(mCurrentFrame.mTcw);
    for(int i=0; i<mpLastKeyFrame->local_ellipsoids.size(); i++)
    {
        MapObject* mObj = mpLastKeyFrame->local_ellipsoids[i];
        g2o::ellipsoid ellipsoid_world = mObj->GetWorldPos();
        Eigen::Vector4d obj_bbox = ellipsoid_world.getBoundingBoxFromProjection(campose_cw,Kalib);
        cv::Rect obj_rect(obj_bbox(0),obj_bbox(1),obj_bbox(2),obj_bbox(3));
        last_rects.push_back(obj_rect);
    }
    bool c3 = false;
    for(int i=0; i<rects.size(); i++)
    {
        ...
        bool new_obj=true;
        for(int j=0; j<last_rects.size(); j++)
        {
            cv::Rect rect2 = last_rects[j];
            cv::Rect A = rect1 | rect2;
            cv::Rect U = rect1 & rect2;
            double iou_2d = U.area()*1.0/A.area();
...
void Tracking::CreateNewKeyFrame()
{
	...
	std::cout << "\033[33m Created new keyframe!  " << pKF->mnId << " local object " << pKF->local_ellipsoids.size() 
    ...
    std::cout << "\033[33m Detect Ellipsoid!  " << pKF->mnId << " local object " << pKF->local_ellipsoids.size()
            << "   total ID  " << pKF->mnFrameId << "\033[0m" << std::endl;

    //
    /*
    if(keyframe_seq.size()>=50)
    {
        KeyFrame* first_kf = keyframe_seq[keyframe_seq.size()-50];
        cv::Mat detect_im = first_kf->raw_img.clone();
        cv::cvtColor(detect_im,detect_im,CV_GRAY2BGR);
        for(int i=0; i<first_kf->local_ellipsoids.size(); i++)
        {
            MapObject* obj = first_kf->local_ellipsoids[i];
            cv::rectangle(detect_im,obj->bbox_2d,cv::Scalar(255,178,50),3);
            int left=obj->bbox_2d.tl().x,top=obj->bbox_2d.tl().y,right=obj->bbox_2d.br().x,bottom=obj->bbox_2d.br().y;
            if(obj->already_associated)
            {
                int id = obj->associated_landmark->mnId;
                string id_str = to_string(id);
                string label = "car "+id_str;
                int baseLine;
                cv::Size labelSize = cv::getTextSize(label,cv::FONT_HERSHEY_SIMPLEX,0.5,1,&baseLine);
                top = max(top,labelSize.height);
                cv::rectangle(detect_im,cv::Point(left,top-round(1.5*labelSize.height)),cv::Point(left+round(1.5*labelSize.width),top+baseLine),cv::Scalar(255,255,255),cv::FILLED);
                cv::putText(detect_im, label,cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 0.75,cv::Scalar(0, 0, 0), 1);
            }
        }
        cv::imshow("detection",detect_im);
        cv::waitKey(0);
    }
    */
			...
			for (size_t i = 0; i < frameMapPointMatches.size(); i++)
			{
				...
				if (pMP && pMP->is_dynamic) // the point is matched to this frame, and also dynamic.
				{
					...
					if (!pMP->is_triangulated) // if not Triangulated
					{
						...
						if (use_dynamic_klt_features)
							objectLastframe = mpLastKeyFrame->local_ellipsoids[mpLastKeyFrame->keypoint_associate_objectID_harris[pixelindLastKf]];
						else
							objectLastframe = mpLastKeyFrame->local_ellipsoids[mpLastKeyFrame->keypoint_associate_objectID[pixelindLastKf]];
						//g2o::cuboid cube_pose_lastkf;
						g2o::ellipsoid ellipsoid_pose_lastkf;
						if (objectLastframe->already_associated)
							ellipsoid_pose_lastkf = objectLastframe->associated_landmark->allDynamicPoses[mpLastKeyFrame].first;
						else
							ellipsoid_pose_lastkf = objectLastframe->GetWorldPos();
						// get new cube pose in this frame??? based on keypoint object asscoiate id.
						MapObject *objectThisframe;
						if (use_dynamic_klt_features)
							objectThisframe = mpCurrentKeyFrame->local_ellipsoids[mCurrentFrame.keypoint_associate_objectID_harris[i]];
						else
							objectThisframe = mpCurrentKeyFrame->local_ellipsoids[mpCurrentKeyFrame->keypoint_associate_objectID[i]];
						//g2o::cuboid cube_pose_now = objectThisframe->GetWorldPos(); //current obj pose, not BA optimimized
						g2o::ellipsoid ellipsoid_pose_now = objectThisframe->GetWorldPos(); 
						...
						g2o::SE3Quat objecttransform = ellipsoid_pose_now.pose * ellipsoid_pose_lastkf.pose.inverse();
						...
						pMP->PosToObj = Converter::toCvMat(ellipsoid_pose_now.pose.inverse().map(Converter::toVector3d(x3D)));
					}
				}
			}
		}
			...
			if (actually_use_obj_depth)
			{
				...
				if (use_dynamic_klt_features)
				{
					...
					for (int new_fea_ind = 0; new_fea_ind < numNewfeat; new_fea_ind++)
					{
						...
						mpCurrentKeyFrame->keypoint_associate_objectID_harris[new_fea_ind + numTracked] = pixelobjid;

						float point_depth = mpCurrentKeyFrame->local_ellipsoids[pixelobjid]->ellipsoid_meas.translation()[2]; // camera z
					...
					if (whether_actually_planeobj_init_pt)
					{
						...
						while ((nPoints < max_initialize_pts) && (vector_counter < (int)has_object_depth_pixel_inds.size()))
						{
							...
							if ((point_depth < 0))
							{
								...
								{
									point_depth = mpCurrentKeyFrame->local_ellipsoids[mpCurrentKeyFrame->keypoint_associate_objectID[pixel_ind]]->ellipsoid_meas.translation()[2]; // camera z
	...
	if (enable_ground_height_scale)
	{
        ...
        std::cout << "Tracking: generate ground: potential_pts: " << pKF->ground_region_potential_pts.size() 
		...
		if (pKF->mnId % ground_everyKFs == 0)
		{
			...
			if (recently_have_object)
				std::cout << "Found ellipsoid landmark in this range" << std::endl;
			if ((!recently_have_object) || (height_esti_history.size() < 1))
			{
				...
				std::cout << "Tracking: Potential plane pt size   " << potential_plane_points.size() 
...
void Tracking::ReadAllObjecttxt()
{
	...
	all_offline_local_objects.reserve(2000);
	...
	for (int img_couter = 0; img_couter < total_img_ind; img_couter++)
	{
		...
		if (read_all_number_txt(pred_frame_obj_txts, pred_frame_objects))
		{
			...
			all_offline_local_objects.push_back(pred_frame_objects);
...
void Tracking::DetectObject(KeyFrame *pKF)
{
	...
	std::vector<ObjectSet> all_objects; // in ground frame, no matter read or online detect
	std::vector<Vector4d> all_obj2d_bbox;
	std::vector<double> all_box_confidence;
	vector<int> truth_tracklet_ids;

	if (whether_read_offline_object) // saved object txt usually is usually poped in local ground frame, not the global ground frame.
	{
		if (all_offline_local_objects.size() == 0)
			return;
		...
		pred_frame_objects = all_offline_local_objects[(int)pKF->mnFrameId];
    ...
    // copy and analyze results. change to g2o ellipsoid.
	pKF->local_ellipsoids.clear();
	...
	for (int ii = 0; ii < (int)all_objects.size(); ii++)
	{
		if (all_objects[ii].size() > 0) // if has detected 3d Cuboid
		{
			cuboid *raw_cuboid = all_objects[ii][0];

            g2o::ellipsoid ellipsoid_ground_value; // offline ellipsoid txt in local ground frame.  [x y z yaw l w h]
			Vector9d ellipsoid_pose;
			ellipsoid_pose << raw_cuboid->pos[0], raw_cuboid->pos[1], raw_cuboid->pos[2], 0, 0, raw_cuboid->rotY,
				raw_cuboid->scale[0], raw_cuboid->scale[1], raw_cuboid->scale[2];
			ellipsoid_ground_value.fromMinimalVector(ellipsoid_pose);

			// measurement in local camera frame! important
			MapObject *newellipsoid = new MapObject(mpMap);
			g2o::ellipsoid ellipsoid_local_meas = ellipsoid_ground_value.transform_to(Converter::toSE3Quat(pop_pose_to_ground));
			newellipsoid->ellipsoid_meas = ellipsoid_local_meas;
			newellipsoid->bbox_2d = cv::Rect(raw_cuboid->rect_detect_2d[0], raw_cuboid->rect_detect_2d[1], raw_cuboid->rect_detect_2d[2], raw_cuboid->rect_detect_2d[3]);
			newellipsoid->bbox_vec = Vector4d((double)newellipsoid->bbox_2d.x + (double)newellipsoid->bbox_2d.width / 2, (double)newellipsoid->bbox_2d.y + (double)newellipsoid->bbox_2d.height / 2,
										   (double)newellipsoid->bbox_2d.width, (double)newellipsoid->bbox_2d.height);
			newellipsoid->box_corners_2d = raw_cuboid->box_corners_2d;
			newellipsoid->bbox_2d_tight = cv::Rect(raw_cuboid->rect_detect_2d[0] + raw_cuboid->rect_detect_2d[2] / 10.0,
			...
			get_cuboid_draw_edge_markers(newellipsoid->edge_markers, raw_cuboid->box_config_type, false);
			newellipsoid->SetReferenceKeyFrame(pKF);
			newellipsoid->object_id_in_localKF = pKF->local_ellipsoids.size();

			g2o::ellipsoid global_obj_pose_to_init = ellipsoid_local_meas.transform_from(frame_pose_to_init);

			newellipsoid->SetWorldPos(global_obj_pose_to_init);
			newellipsoid->pose_noopti = global_obj_pose_to_init;
			if (use_truth_trackid)
				newellipsoid->truth_tracklet_id = truth_tracklet_ids[ii];

			if (scene_unique_id == kitti)
			{
				if (ellipsoid_local_meas.pose.translation()(0) > 1)
					newellipsoid->left_right_to_car = 2; // right
				if (ellipsoid_local_meas.pose.translation()(0) < -1)
					newellipsoid->left_right_to_car = 1; // left
				if ((ellipsoid_local_meas.pose.translation()(0) > -1) && (ellipsoid_local_meas.pose.translation()(0) < 1))
					newellipsoid->left_right_to_car = 0;
			}
			if (1)
			{
				double obj_cam_dist = std::min(std::max(newellipsoid->ellipsoid_meas.translation()(2), 10.0), 30.0); // cut into [a,b]
				double obj_meas_quality = (60.0 - obj_cam_dist) / 40.0;
				newellipsoid->meas_quality = obj_meas_quality;
			}
			else
				newellipsoid->meas_quality = 1.0;
			if (all_box_confidence[ii] > 0)
				newellipsoid->meas_quality *= all_box_confidence[ii]; // or =

			if (newellipsoid->meas_quality < 0.1)
				std::cout <<"Abnormal measure quality!!:   " << newellipsoid->meas_quality << std::endl;
			pKF->local_ellipsoids.push_back(newellipsoid);
		}
	}

	// std::cout << "Tracking: created local object num   " << pKF->local_cuboids.size() << std::endl;
	std::cout << "Tracking: detect ellipsoid for pKF id: " << pKF->mnId << "  total id: " << pKF->mnFrameId << "  numObj: " << pKF->local_ellipsoids.size() << std::endl;

	if (whether_save_online_detected_objects)
	{
		for (int ii = 0; ii < (int)all_objects.size(); ii++)
		{
			if (all_objects[ii].size() > 0) // if has detected 3d Cuboid, always true in this case
			{
				cuboid *raw_cuboid = all_objects[ii][0];
				g2o::ellipsoid ellipsoid_ground_value;
				Vector9d ellipsoid_pose;
				ellipsoid_pose << raw_cuboid->pos[0], raw_cuboid->pos[1], raw_cuboid->pos[2], 0, 0, raw_cuboid->rotY,
					raw_cuboid->scale[0], raw_cuboid->scale[1], raw_cuboid->scale[2];
				save_online_detected_objects << pKF->mnFrameId << "  " << ellipsoid_pose.transpose() << "\n";
			}
		}
	}
	...
	if (associate_point_with_object)
	{
		...
		if (!whether_dynamic_object) //for old non-dynamic object, associate based on 2d overlap... could also use instance segmentation
		{
			...
			std::vector<bool> overlapped(pKF->local_ellipsoids.size(), false);
			if (1)
			{
				for (size_t i = 0; i < pKF->local_ellipsoids.size(); i++)
					if (!overlapped[i])
						for (size_t j = i + 1; j < pKF->local_ellipsoids.size(); j++)
							if (!overlapped[j])
							{
								float iou_ratio = bboxOverlapratio(pKF->local_ellipsoids[i]->bbox_2d, pKF->local_ellipsoids[j]->bbox_2d);
								if (iou_ratio > 0.15)
								{
									overlapped[i] = true;
									overlapped[j] = true;
			...
			if (!enable_ground_height_scale)
			{									   // slightly faster
				if (pKF->local_ellipsoids.size() > 0) // if there is object
					for (size_t i = 0; i < pKF->mvKeys.size(); i++)
					{
						int associated_times = 0;
						for (size_t j = 0; j < pKF->local_ellipsoids.size(); j++)
							if (!overlapped[j])
								if (pKF->local_ellipsoids[j]->bbox_2d.contains(pKF->mvKeys[i].pt))
								{
									associated_times++;
			...
			else
			{
                ...
                if (height_esti_history.size() == 0 && !mono_firstframe_truth_depth_init)
				{
	                std::cout << "Tracking: height_esti_history.size() = 0, clear object  " << std::endl;
					pKF->local_ellipsoids.clear(); // don't do object when in initial stage...
		...
		if (whether_dynamic_object) //  for dynamic object, I use instance segmentation
		{
			if (pKF->local_ellipsoids.size() > 0) // if there is object
			{
				...
				for (size_t i = 0; i < pKF->mvKeys.size(); i++)
				{
					if (pKF->keypoint_associate_objectID[i] >= 0 && pKF->keypoint_associate_objectID[i] >= pKF->local_ellipsoids.size())
					{
                        std::cout <<"Detect ellipsoid find bad pixel obj id  " << pKF->keypoint_associate_objectID[i] << "  " << pKF->local_ellipsoids.size() << std::endl;
					}
	...
	std::cout << "Tracking: check the objects has enough points, frame num:" << checkframes.size() << std::endl;
	for (size_t i = 0; i < checkframes.size(); i++)
	{
		KeyFrame *kfs = checkframes[i];
		for (size_t j = 0; j < kfs->local_ellipsoids.size(); j++)
		{
			MapObject *mPO = kfs->local_ellipsoids[j];
			if (!mPO->become_candidate)
			{
	            ...
	            // std::cout << "mPO->become_candidate  " << mPO->become_candidate
                // << " object_own_point_threshold  " << object_own_point_threshold << std::endl;
			}
		}
	}
}
	
void Tracking::AssociateObjects(KeyFrame *pKF)
{
	...
	std::cout << "Tracking: AssociateObjects  " << "mvpLocalKeyFrames.size()  " << mvpLocalKeyFrames.size() << std::endl;
	...
	for (size_t i = 0; i < mvpLocalKeyFrames.size(); i++) // pKF is not in mvpLocalKeyFrames yet
	{
		...
		for (size_t j = 0; j < kfs->local_ellipsoids.size(); j++)
		{
			...
			if (mPO->become_candidate && (!mPO->already_associated))
				LocalObjectsCandidates.push_back(kfs->local_ellipsoids[j]);
		}

		for (size_t j = 0; j < kfs->ellipsoids_landmark.size(); j++)
			if (kfs->ellipsoids_landmark[j]) // might be deleted due to badFlag()
				if (!kfs->ellipsoids_landmark[j]->isBad())
					if (kfs->ellipsoids_landmark[j]->association_refid_in_tracking != pKF->mnId) // could also use set to avoid duplicates
					{
						LocalObjectsLandmarks.push_back(kfs->ellipsoids_landmark[j]);
						kfs->ellipsoids_landmark[j]->association_refid_in_tracking = pKF->mnId;
					}
	}

	std::cout << "Tracking: Associate ellipsoids #candidate: " << LocalObjectsCandidates.size() << " #landmarks " << LocalObjectsLandmarks.size()
	...
	for (size_t i = 0; i < LocalObjectsCandidates.size(); i++)
	{
		...
		if (largest_shared_objectlandmark == nullptr) // if not found, create as new landmark.  either using original local pointer, or initialize as new
		{
			...
			refframe->ellipsoids_landmark.push_back(candidateObject);
			...
			{
				g2o::ellipsoid ellipsoidglobalpose = candidateObject->GetWorldPos();
                //ellipsoidglobalpose.setScale(Eigen::Vector3d(1.9420, 0.8143, 0.7631));
                ellipsoidglobalpose.setScale(Eigen::Vector3d(car_half_length,car_half_width,car_half_height));
				candidateObject->SetWorldPos(ellipsoidglobalpose);
				candidateObject->pose_Twc_latestKF = ellipsoidglobalpose;
				candidateObject->pose_noopti = ellipsoidglobalpose;
				candidateObject->allDynamicPoses[refframe] = make_pair(ellipsoidglobalpose, false); //Vector6d::Zero()  false means not BAed
			}
		...
		else // if found, then update observation.
		{
            ...
            KeyFrame *refframe = candidateObject->GetReferenceKeyFrame();
            /*
            g2o::SE3Quat campose_cw = Converter::toSE3Quat(refframe->GetPose());
            g2o::ellipsoid ellipsoid_world;
            if(!largest_shared_objectlandmark->averaged)
                ellipsoid_world = largest_shared_objectlandmark->GetWorldPos();
            else
                ellipsoid_world = largest_shared_objectlandmark->pose_avg;
            Eigen::Vector4d landmark_bbox = ellipsoid_world.getBoundingBoxFromProjection(campose_cw,Kalib);
            cv::Rect landmark_rect(landmark_bbox(0),landmark_bbox(1),landmark_bbox(2),landmark_bbox(3));
            cv::Rect candidate_rect = candidateObject->bbox_2d;
            cv::Rect A = landmark_rect | candidate_rect;
            cv::Rect U = landmark_rect & candidate_rect;
            double iou_2d = U.area()*1.0/A.area();
            if(iou_2d<0.1)
            {
                largest_shared_objectlandmark = nullptr;
            ...
            */
			...
			refframe->ellipsoids_landmark.push_back(largest_shared_objectlandmark);
			...
			if (scene_unique_id == kitti)
			{
				...
				largest_shared_objectlandmark->allDynamicPoses[refframe] = make_pair(ellipsoidglobalpose, false);
				largest_shared_objectlandmark->SetWorldPos(ellipsoidglobalpose);
				largest_shared_objectlandmark->pose_Twc_latestKF = ellipsoidglobalpose; //if want to test without BA
				largest_shared_objectlandmark->pose_noopti = ellipsoidglobalpose;
			}
			largest_shared_objectlandmark->MergeIntoLandmark(candidateObject);

            //
            largest_shared_objectlandmark->UpdateRotation(candidateObject);
		}
	}
	...
	if (remove_object_outlier)
	{
		...
		for (size_t i = 0; i < all_objects.size(); i++)
		{
			...
			if ((!pMObject->isBad()) && (!pMObject->isGood))						// if not determined good or bad yet.
                if ((int)pMObject->GetLatestKeyFrame()->mnId < (int)pKF->mnId - 20) //20
				{
                    ...
                    bool no_enough_inlier_pts = check_object_points && (pMObject->NumUniqueMapPoints() < 10);
                    if (pMObject->Observations() < minimum_object_observation  ||  no_enough_inlier_pts)
					{
						pMObject->SetBadFlag();
