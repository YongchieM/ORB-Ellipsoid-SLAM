...
void MapPoint::AddObservation(KeyFrame* pKF, size_t idx)
{
    ...
    if (associate_point_with_object)
    {
        if (use_dynamic_klt_features && is_dynamic)
        {
            ...
            {
                int frame_object_id = pKF->keypoint_associate_objectID_harris[idx];
                if (frame_object_id > -1)
                    AddObjectObservation(pKF->local_ellipsoids[frame_object_id]);
            }
        }
        else
        {
            ...
            {
                int frame_object_id = pKF->keypoint_associate_objectID[idx];
                if (frame_object_id > -1)
                    AddObjectObservation(pKF->local_ellipsoids[frame_object_id]);
            }
        }
